...
def getLaneCurve(img):
    ...
    #STEP 2
    hT, wT, c = img.shape
    points = np.float32([(60, 175), (wT-60, 175),(80, 230), (wT-80, 230)])
    imgWarp = warpImg(imgThres, points, wT, hT)

    #STEP 3
    middlePoint,imgHist = getHistogram(imgWarp, display=True, minPer=0.5, region=6)
...
#     cv2.imshow('Hist', imgHist)
    img = stackImages(0.7, [imgThres, imgWarp, imgHist, imgResult])
    # print(curve)
    return curve,img
...
#         x = steering_control(curve)
#         z = float(Decimal(x).quantize(Decimal('0.1'), rounding=ROUND_UP))
#         print(x)
#         print(curve)
